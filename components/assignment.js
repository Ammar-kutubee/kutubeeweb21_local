    ...
    return (
        ...
        >
            ...
            <div
                ...
                style={{width:"19%"}}
                className="assignmentWrapper">
                <div className="flex-wrapper-row">
                    <div style={{ position: 'relative' }}>
                        {item.books.length != 0 &&
                            <img
                                src={item.books[0].coverPhoto}
                    ...
                    <div className="assignmentInfoWrapper">
                        <div className="teacherData">
                            {
                                item.avatarlink == ''
                                    ...
                                    <img style={{ borderWidth: 1 }}
                                        src={item.avatarlink ? item.avatarlink : '../assets/images/bee.png'} alt="Avatar Image"
                                    />
                                    :
                        ...
                        <div>
                            <div numberOfLines={3} className="description">{item.name}</div>
                        </div>
                    </div>
                    <div>
                    </div>
                ...
                <div className="rightInfoWrapper">
                    <div style={{
                        ...
                        {type == "active" &&
                            <div className='dueDate'> {moment.unix(item.dueDate).format("D MMM")}</div>
                        }

                        {type == "completed" &&
                            <div className='semiBoldText'> {moment.unix(item.dueDate).format("D MMM")}</div>
                        }
                        {type == "over" &&
                            <div className='overdate'> {moment.unix(item.dueDate).format("D MMM")}</div>
                        }
                    </div>
                    <div>
                        <div style={{
                            ...
                            <div name="right-arrow" className="viewArrow icon-right-arrow" > </div>
                        </div>
                    </div>
                </div>
            </div >
